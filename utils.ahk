...
MyRun(AppDefinitionFn)
{
    AppDefinition := %AppDefinitionFn%()
    WinTitle := AppDefinition[1]
    Target := AppDefinition[2]

    Run, %Target%
}

MyApp2(AppDefinitionFn, LayoutFunc := "")
{
    AppDefinition := %AppDefinitionFn%()
    WinTitle := AppDefinition[1]
    Target := AppDefinition[2]
...
MyMusic(query)
{
    ...
    WinWait, %WinTitle%
    Sleep, 150
    www := "https://www.youtube.com/results?search_query="
    Send %www%%query%
    Send, {Enter}
}
